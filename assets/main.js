    ...
    try {
       ...
       let view = `
        ...
        <div class="group relative">
            ...
            <div class=" flex flex-col relative justify-between bg-orange-600 p-3 rounded-b-lg">
                    <div class="mb-4">
                        <h1 class="font-extrabold text-gray-50 sm:text-2xl md:text-2xl">${character.name}</h1>
                        <span class=" text-black">${character.status} - ${character.species}</span>
                    </div>

                    <div class="mb-2">
                        <h3 class="font-extrabold text-gray-50">Last known location:</h3>
                        <span class="text-black">${character.location.name}</span>
                    </div>
            </div>
        </div>
        `).join('')}
